...
def save_uploaded_file(uploaded_file) -> Path | None:
    ...
    if dest.exists():
        dest = (
            Path(settings.upload_dir) / f"{dest.stem}_{uuid.uuid4().hex}{dest.suffix}"
        )
    try:
...
if page == "Upload":
    ...
    uploaded = st.file_uploader(
        "Upload audio", type=["wav", "mp3", "m4a", "webm", "flac"]
    )
    model = st.selectbox(
        "Model",
        options=[
                    ...
                    try:
                        res = asyncio.run(
                            transcription_service.transcribe_file(
                                dest, model_name=model
                            )
                        )
                        st.success("Transcription complete")
                        st.text_area(
                            "Transcript", value=res.get("text", ""), height=200
                        )
                        st.download_button(
                            "Download transcript",
                            data=res.get("text", ""),
                            file_name=f"{dest.stem}.txt",
                        )
